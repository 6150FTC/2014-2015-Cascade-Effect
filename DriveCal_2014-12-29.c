...
task main()
{
    ...
    float percentOnLeft = .80;
    float driveSpeed = 70;
    waitForStart();
    nMotorEncoder[left] = 0;
    nMotorEncoder[right] = 0;
    drive((percentOnLeft*driveSpeed), driveSpeed);
    while(nMotorEncoder[left] < 8000)
    {
     }
   driveStop();
   while(true)
    {
    sprintf(buffer, "%d", nMotorEncoder[left]);
        nxtDisplayBigTextLine(1 , buffer );
    sprintf(buffer, "%d", nMotorEncoder[right]);
        nxtDisplayBigTextLine(3 , buffer );
    } // Infinite while so that nxtDisplay does not get overwritten
}// end task main
