...
{
    float balanceAdjustmentPercent = .50; //percent to adjust power to counter weight imbalance. float based on power of 70;
...
if(turn_deg > 0)
    {
         whichEncoder = right;
         drive(0,speed_pct);
         multiplyer = 37;//41 was the previous multiplyer until tightening of treads changed calibration
    }
    else
    {
        drive(speed_pct,0);
...
void openScoopGate(int state)
{
    if(state == 1)
    {
        servo[ScoopGate] = 80;//Scoop gate up
    }
    else if(state == 0)//
    {
    ...
    {
        servo[ScoopGate] = 150;//Scoop gate down
    }
}
